...
The current snapshot joins Epoch's registry to its timeline, identifies U.S. sites, and reconstructs the latest state at each quarterly cutoff. U.S. IT power rises from **0.473 GW in 2024-Q1 to 11.879 GW on 26 August 2026**. The same states contain **13.524 million H100e** at the current cutoff; that value is retained as a secondary bridge for estimating productivity, not as the gate unit. The base forward path uses Epoch's dated expected/projected site states rather than extending a historical doubling-time regression. Mean projected IT-power growth is **0.157207 log₂ GW per quarter**, with acceleration **−0.001503 log₂ GW per quarter²**. [Epoch registry CSV](https://epoch.ai/data/data_centers/data_centers.csv), [Epoch timeline CSV](https://epoch.ai/data/data_centers/data_center_timelines.csv)

The H100e-to-IT-power ratio and the disclosed serving envelope yield **227.101 trillion reference token-equivalents per IT GW-day** at the current cutoff. Across the forward source path, mean productivity growth is **0.086584 log₂ per quarter**, with acceleration **+0.004876 log₂ productivity per quarter²**. Separating these series prevents a fast chip generation from being mistaken for more physical power and prevents a fast buildout from being mistaken for better inference efficiency.
...
The current snapshot derives `η_ref` from H100e per IT GW and a reference serving envelope. Do not multiply that productivity again by a chip-generation claim already embodied in the dated H100e state. Add a separate productivity change only for measured deployed serving gains not captured by the bridge—memory bandwidth, interconnect, latency-aware scheduling, caching, quantization, software, or a new architecture under a comparable service level.
...
4. Append monthly operational IT-power and reference-token-productivity snapshots, preserving H100e as a secondary audit bridge; refit their log slopes separately.
