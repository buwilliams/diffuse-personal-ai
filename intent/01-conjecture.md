...
| Desktop computer use | On the original OSWorld family, OpenAI’s CUA scored **38.1%** in 2025; GPT-5.5 later scored **78.7%** on OSWorld-Verified. On the harder OSWorld 2.0, OpenAI reports **72.6% partial credit** for GPT-6 Astra and Anthropic reports **77.9% partial / 41.7% strict** for Fable 5.1. In the benchmark paper’s full/binary measure, Claude Opus 4.8 completed only **20.6%** of workflows even with 500 steps. | GUI competence has risen rapidly, but partial credit can conceal failure to complete the user’s goal. Vendor configurations, partial scoring, strict completion, and OSWorld v1/Verified are not one continuous series. [OSWorld 2.0](https://osworld-v2.xlang.ai/), [GPT-6 Astra](https://openai.com/index/gpt-6-astra/), [Fable 5.1](https://www.anthropic.com/claude-fable-and-mythos-5-1)
...
| Broad real work | Agents’ Last Exam contains more than 1,500 expert tasks across 55 non-physical occupations. The best broad ALE-CLI result reported at launch was **25.2%**, and every agent scored **0%** on the hardest tier. OpenAI's current release table reports **53.6% for GPT-5.6 Sol and 59.3% for GPT-6 Astra**. | Narrow software benchmarks substantially overstate “all digital work.” The launch CLI and current provider figures should not be silently spliced, but the same-table 5.7-point generational gain is useful. Common failures still include declaring success without verifying the requested artifact. [Berkeley RDI](https://rdi.berkeley.edu/blog/agents-last-exam/), [GPT-6 Astra](https://openai.com/index/gpt-6-astra/)
...
Epoch AI's 3 September data release contains **86 sites, 13.94 million H100-equivalents, and 13.26 GW of current IT power**. The forecast joins its registry to its timeline and reconstructs **12.109 GW of operational U.S. IT power** at the 3 September cutoff. The same states contain **13.046 million H100e**, retained only as a transparent audit cross-check. Inference productivity comes from an independent MLPerf goodput-per-power series. This is public-tracker capacity, not a complete census, and the September release revised historical and projected H100e estimates without allowing that audit field to change service capacity.
...
Specialized inference architectures enter the inference-productivity path only when their deployed share and comparable serving result are known. Cerebras' CS-4 claim of up to **30× faster inference** is a speed/interactivity result. OpenAI's Jalapeño exceeds **50× throughput per kilowatt** at matched previous-best GPU time-between-tokens on three workloads, but its general peak advantage is **1.5–1.9× work per watt**. The forecast therefore varies deployment share and workload-specific productivity instead of applying 30× or 50× to the whole fleet. [Cerebras CS-4](https://www.cerebras.ai/cs4), [OpenAI Jalapeño](https://openai.com/index/jalapeno-first-results/)
...
- **+5 points:** the twelve-month operational U.S. IT-power series accelerates to a doubling time below 120 days, with commissioned—not planned—capacity and no material deterioration in inference allocation or availability.
...
- **−5 points:** the twelve-month operational U.S. IT-power doubling time rises above 365 days, or multiple capacity milestones slip by more than two quarters without replacement supply.
